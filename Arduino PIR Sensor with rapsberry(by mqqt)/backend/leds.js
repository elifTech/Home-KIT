...
clientMosquitto.on('connect', function () {
	...
	var ledArray=[{"sensor":{"name":"yellow","value":0}},{"sensor":{"name":"green","value":250}},{"sensor":{"name":"red","value":0}}];
	var ledState = {"state":{"reported":{"leds_set":ledArray}}};

	thingShadows.on('connect', function() {
	console.log('connceted to AWS'+ new Date())
	...
	thingShadows.on('status',
		...
		console.log("RESPONSE FROM AMAZON RECIEVED" + new Date());
	...
	//	console.log("Decoded msg:" +decode(encodedState));
		clientMosquitto.publish('led/status', JSON.stringify({"iv":iv,"message": encodedState}) );
	    });

	thingShadows.on('delta',
	    function(thingName, stateObject) {
		console.log('DELTA EVENT '+ new Date());
		console.log('Thing name: '+ thingName);
		var leds_set=stateObject.state.leds_set;
		console.log('State object: '+ leds_set);
		thingShadows.update('Led', {"state":{"reported":{"leds_set":leds_set}}});
	    });
